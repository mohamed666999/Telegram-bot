...
async def analyze(bonus, suit, hand):
    ...
    return report, gemini_res, nvidia_res, rec_model

# ==================== أوامر البوت ====================
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data.clear()
    count = db_fetch_one("SELECT COUNT(*) as c FROM history")['c'] or 0
    kb = [
...
async def stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    total = db_fetch_one("SELECT COUNT(*) as c FROM history")['c'] or 0
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("result_"):
        ...
        save_prediction_result(
            context.user_data['last_bonus'], context.user_data['last_suit'], "متنوع",
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text.isdigit() and len(text) in (7, 8):
        ...
        report, gemini_pred, nvidia_pred, rec_model = await analyze(
            text, context.user_data['last_suit'], "متنوع"
        )
