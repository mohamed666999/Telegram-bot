...
def ask_ai(model_name, api_key, prompt):
    # تمييز الرابط الرسمي لـ xAI عن روابط OpenRouter
    ...
    try:
        ...
        # إضافة Header إضافي مطلوب لـ OpenRouter أحياناً (لا يضر xAI)
        headers["HTTP-Referer"] = "https://railway.app"
        
        resp = requests.post(
            url,
            headers=headers,
            json={"model": model_name, "messages": [{"role": "user", "content": prompt}], "temperature": 0.2, "max_tokens": 100},
            timeout=14 # مهلة كافية للتحليل الرياضي
        )
        
        if resp.status_code == 200:
            return resp.json()['choices'][0]['message']['content'].strip()
        elif resp.status_code == 402:
            return "⚠️ خطأ 402: الرصيد نفد"
        else:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("🏛️ **الكيان V73.0 (Quantum Core)**\nالنظام جاهز للتحليل الرباعي. اختر النوع:", reply_markup=InlineKeyboardMarkup(kb))
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text.isdigit() and len(text) in [7, 8]:
        ...
        msg = await update.message.reply_text("🧬 **جاري دمج البيانات وتوليد القاعدة الرياضية...**")
        
        now = datetime.datetime.now()
        current_time_str = now.strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]
        
        # استخراج البيانات التاريخية للتدريب اللحظي
        raw_data = ""
        try:
            conn = psycopg2.connect(DATABASE_URL, sslmode='require')
            with conn.cursor() as cur:
        ...
        prompt = f"""أنت محرك رياضي Quantum. حلل البيانات الزمنية الخام التالية لابتكار قاعدة:
{raw_data}
...
التوقع: 🔵 ثور (أو 🔴 راعي)
الثقة: %
القاعدة: (اشرح القانون المبتكر باختصار)"""

        # تشغيل المحاكاة الرباعية
        ...
        clean_res = []
        for r in results:
            if isinstance(r, Exception): clean_res.append("⚠️ عطل في الخادم")
            else: clean_res.append(r)
        
        r1, r2, r3, r4 = clean_res

        report = (f"⏰ **التوقيت الدقيق:** `{current_time_str}`\n━━━━━━━━━━━━\n"
                  f"🧠 **Gemini:**\n{r1}\n\n"
                  f"🤖 **Nvidia:**\n{r2}\n\n"
                  f"🔍 **Claude:**\n{r3}\n\n"
                  f"🌌 **xAI (Grok):**\n{r4}\n━━━━━━━━━━━━\n"
