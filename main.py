"""
HADES V112.1 - The Oracle (Bug Fixes & Stable Workflow)
إصلاح مشكلة تعليق البوت بعد الجولة الأولى وإصلاح زر التنزيل.
"""
...
def predict_3_layer(b_num: str, suit: str, rank: str) -> Tuple[int, str]:
    ...
    if not digits: return 2, "❌ رقم غير صالح"
...
async def download_db(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_document(document=open(filename_hist, 'rb'), caption="📊 جدول التاريخ (History)")
        await update.message.reply_document(document=open(filename_laws, 'rb'), caption="⚖️ جدول القوانين (Laws)")
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if data == "choose_suit":
            ...
            kb = [
                [InlineKeyboardButton("🗑️ مسح (تراجع)", callback_data="delete_last")],
                [InlineKeyboardButton("🔄 تغيير الورقة", callback_data="choose_suit")]
            ]
            await query.edit_message_text(f"✅ تم التسجيل بنجاح: **{WINNER_NAMES[w_code]}**\n\n📥 **أرسل الرقم التالي لنفس الورقة ({suit} {rank})**، أو اضغط تغيير.", reply_markup=InlineKeyboardMarkup(kb), parse_mode='Markdown')
            
    except Exception as e:
        logger.error(f"Callback Error: {e}")
        await query.edit_message_text(f"❌ حدث خطأ بسيط. أرسل الرقم مجدداً أو اضغط /start")
...
if __name__ == "__main__":
    ...
    logger.info("🚀 HADES V112.1 Is Online and Stable!")
