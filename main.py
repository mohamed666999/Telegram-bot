...
def get_latest_stats(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    total = len(df)
    stats['bias'] = {
        'red': float((df['winner'] == 'الراعي 🔴').sum() / total) if total > 0 else 0,        'blue': float((df['winner'] == 'الثور 🔵').sum() / total) if total > 0 else 0,
    ...
    if 'b_num' in df.columns:
        df_copy = df.copy()
        df_copy['last_digit'] = df_copy['b_num'].astype(str).str[-1].str.extract('(\d)')
        digit_winner = {}
...
def load_active_equation() -> str:
    try:
        conn = get_db_connection()
        cur = conn.cursor()        cur.execute("""SELECT equation_code FROM ai_equations
        ...
        for p in ['morning', 'afternoon', 'evening', 'night']:
            ...
            if len(pd_data) >= min_samples:
                ...
                probs[p] = (
                    ...
                    (pd_data['winner_code'] == 1).sum() / t,                    (pd_data['winner_code'] == 2).sum() / t,
                )
            else:
                probs[p] = None
        return probs
...
def apply_ai_laws(b_num: str, suit: str, gap: int, bayes_probs: Optional[Dict]) -> Optional[Tuple[str, int]]:
    ...
    for law in laws:
        ...
        if score >= 0.5:
            ...
            if winner in WINNER_MAP:                logger.info(f"🎯 قانون مطبق: {law['name']}")
    ...
    async def deep_review(self, context: ContextTypes.DEFAULT_TYPE):
        ...
        if ai_result:
            ...
            if ai_result.get('config_suggestions'):
                ...
                if safe:                    save_dynamic_config(safe)
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "choose_suit":
        kb = [[InlineKeyboardButton(s, callback_data=f"s_{s}") for s in SUITS[:2]],
              [InlineKeyboardButton(s, callback_data=f"s_{s}") for s in SUITS[2:]],              [InlineKeyboardButton("🔙 رجوع", callback_data="start_back")]]
...
• ⚪: {stats['bias'].get('tie', 0):.1%}"""
        ...
        kb = [
            [InlineKeyboardButton("📊 تحليل شامل", callback_data="admin_analyze")],            [InlineKeyboardButton("🔄 إعادة تحميل", callback_data="admin_reload")],
            [InlineKeyboardButton("🔙 الرئيسية", callback_data="start_back")]
        ]
...
• انحياز 🔴: {stats['bias'].get('red', 0):.1%}"""
        ...
        if len(parts) >= 4:
            ...
            except Exception as e:                logger.error(f"❌ خطأ الحفظ: {e}")
