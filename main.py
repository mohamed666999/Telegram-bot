"""
HADES V16.5 - THE NEURAL HYBRID (DeepSeek-V3.2 via NVIDIA)
تم تحديث الإعدادات لاستخدام نموذج deepseek-ai/deepseek-v3.2 عبر NVIDIA API.
"""
...
class CustomAIEngine:
    def __init__(self):
        self.client = AsyncOpenAI(
            ...
            timeout=10.0  # زيادة المهلة لأن النموذج قد يكون أبطأ
        )
    ...
    async def get_prediction(self, recent_history: list) -> Tuple[Optional[int], float, str]:
        ...
        """
        try:
            # استخدام Stream=True لجمع الرد
            stream = await self.client.chat.completions.create(
                model=AI_MODEL,
                messages=[{"role": "user", "content": prompt}],
                temperature=1.0,          # كما في الكود الأصلي
                top_p=0.95,
            ...
            async for chunk in stream:
                ...
                # قد يكون هناك reasoning_content خاص بـ DeepSeek
                if hasattr(delta, "reasoning_content") and delta.reasoning_content:
                    reasoning_content += delta.reasoning_content
                if delta.content:
                    full_content += delta.content

            # دمج التفكير مع المحتوى للتسجيل
            ...
            if data:
                ...
                logger.warning(f"لم نتمكن من استخراج JSON من الرد: {full_content[:200]}")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "دمج النماذج الإحصائية مع DeepSeek-V3.2.\n"
        "اضغط للبدء:",
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if not suit or not rank:
            ...
            return
        processing_msg = await update.message.reply_text(f"⏳ <b>يتم استشارة الإحصائيات و {AI_MODEL}...</b>", parse_mode='HTML')
...
if __name__ == "__main__":
    ...
    logger.info("🚀 HADES V16.5 (DeepSeek-V3.2) is running...")
