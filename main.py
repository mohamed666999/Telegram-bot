...
🔢 جولات: {stats.get('total_rounds', 0)}🏆 توزيع:
                ...
                try:
                    ...
                    if conn:                        cur = conn.cursor()
                        ...
                        """, (
                            ...
                            WINNER_MAP.get(winner.split()[0], -1),
                            user_id
                        ))
                        conn.commit()
                        conn.close()
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if context.user_data.get('awaiting_bonus') and text.isdigit() and len(text) >= 7:
            ...
            ai_engine = AdvancedAIEngine()            result = ai_engine.generate_prediction_command(df, text, suit)
