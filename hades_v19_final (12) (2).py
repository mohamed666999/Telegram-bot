الخطأ الذي ظهر لك:
`cannot access local variable 'used' where it is not associated with a value`

**سببه واضح جداً:** ملف الكود لديك (`main.py`) أصبح يحتوي على نسخ **متكررة ومتداخلة** من نفس الدوال بسبب عمليات النسخ واللصق. دالة `apply_laws` موجودة في ملفك 3 أو 4 مرات! وفي إحداها تم وضع شرط `if used >= 10` قبل أن يتم تعريف `used = law.get("times_used", 0)`.

لحل هذه "الفوضى البرمجية" بالكامل وتفعيل محرك **Ensemble Probabilistic Model** بأوزانه المثالية الخالية من الأخطاء والهلوسات، يجب تصفير الكود.

### ⚠️ التعليمات (مهم جداً):
1. افتح ملف الكود الخاص بك (`main.py` أو أياً كان اسمه).
2. حدد **كــــل شيء** فيه (Ctrl + A) ثم احذفه (Delete).
3. انسخ الكود الذهبي بالأسفل وضعه في الملف ككود نظيف تماماً.

---

### 💻 الكود النهائي المكتمل (HADES V21)
